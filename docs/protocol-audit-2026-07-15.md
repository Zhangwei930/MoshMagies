...
- `cargo test --all-targets`：325 项通过、0 项失败；四个需要真实服务端的 live test 默认 ignored，并已再次从本机连接 Ubuntu 官方服务端逐项执行通过；
...
- Netcatty 完整自动测试以受控并发运行：5,157 项通过、0 项失败、3 项按平台跳过；SSH 启动和交接的 52 项定向测试全部通过。
...
| #2121 现象 | 本审计能确认的边界 |
|---|---|
| `no MOSH CONNECT` | 这是 SSH bootstrap/输出解析阶段，发生在 MoshCatty 网络客户端启动前。对照官方启动器后确认 Netcatty 曾省略官方默认的强制远端 PTY，这会削弱 Windows ConPTY 下连接行排空保证；现已恢复官方 `-n -tt` 行为，分段、无换行、退出收尾和会话交换测试均通过，Ubuntu 连续启动 50 次全部收到连接行。仍需在报告问题的 Windows 机器上做最终确认 |
