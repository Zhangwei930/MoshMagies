...
- `cargo test --all-targets`：327 项通过、0 项失败；四个需要真实服务端的 live test 默认 ignored，并已再次从本机连接 Ubuntu 官方服务端逐项执行通过；
...
- 最新改动在同一台 Ubuntu 机器上分别切换官方 Mosh 1.3.2 和 1.4.0，完整重跑四项真实会话。输入去重、错误候选地址后的自动选路、远端进程启动尺寸、TAB 画面、缩放、超过 1 MiB 的单条压缩指令和 12 秒静默恢复全部通过。官方服务端只发送一次最终关闭确认后立即退出，因此测试同时核对“收到确认”或“整个远端会话确实结束”，避免把末包丢失误报为远端残留。
