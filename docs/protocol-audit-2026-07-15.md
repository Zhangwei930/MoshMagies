...
本审计把 MoshMagies 与最新版 `mobile-shell/mosh`、Mosh 论文和 RFC 7253 逐项对照，并在用户提供的 Ubuntu 24.04 公网机器上，以系统自带的官方 `mosh-server 1.4.0` 为服务端，从本地 macOS 运行 MoshMagies 做真实互通测试。文中的“修复前”数据来自提交 `8cb4f53`，“修复后”数据来自本次工作树。
...
最终又把官方 1.4.0 的终端状态机做成确定性差分生成器，在 macOS 和 Ubuntu 各执行 5,000 组、每组 120 次终端操作。该测试发现三种此前单元测试没有覆盖的宽字符问题：写入宽字符续格、从续格开始清除，以及 Ubuntu 与 xterm.js 对 emoji 宽度判断不一致时后续画面左移；最终代码复核又补出两个相邻宽字符相互覆盖的尾格清理。四项均已修复并留下最小回归测试。MagiesTerminal 的 SSH 启动也恢复为官方默认的 `-n -tt` 方式，确保服务端在启动进程退出前排空 `MOSH CONNECT` 行；Ubuntu 连续 50 次启动均收到连接行。

本轮继续发现两项与 #2121 直接相关的边界：当前客户端原先会在一次轮询中无限收空 UDP 队列，持续刷屏或无效数据洪泛可能拖住键盘；MagiesTerminal 也没有像官方启动器一样把 SSH 实际连接的服务器地址交给 UDP 客户端，域名有多个地址时可能把 UDP 发到另一地址。现已把每轮收包设为公平且有硬上限，并让 MagiesTerminal 同时提供 SSH 实际地址和原主机名作为候选，客户端向候选地址启动、收到首个合法状态后锁定有效路径。Ubuntu 官方 1.3.2 和 1.4.0 均完成了真实互通验证。

本轮进一步把官方服务端的连续画面、MoshMagies 重绘和 MagiesTerminal 实际使用的 xterm.js 放在同一条链路对照。新测试又找到并修复三个右边界问题：末格字符后的退格、宽字符覆盖后露出的底色，以及官方认为两格但页面认为一格的表情落在末格时被错误换行。复审又发现 Unicode 版本之间有 88 个宽度差异，已按 MagiesTerminal 固定使用的 Unicode 15 规则补齐，并逐一对照全部 1,112,064 个有效码点，右边界判断为 0 差异。20,000 组不含宽字符的连续/最终画面对照为 0 差异；更严格的完整产品链路又执行 2,000 组、每组 180 次操作，MoshMagies 与官方画面在 xterm.js 上全部一致。对照同时确认，不能把“官方一次性最终画面”直接当作所有宽字符分段画面的判定标准：官方 Display 会省略藏在宽字符续格下的内容，服务端和页面也会对部分字符宽度作出不同判断。客户端无法凭未收到的信息恢复这种理想最终画面，但本轮完整产品链路没有出现用户可见差异。
...
| MoshMagies | 修复链起点 `8cb4f53987e584ad9130d6e2edd225ac1f1b01a3`；补充复查固定点 `f2bb6ad7070d4f81ffd5c3592c98c94c75a39950`；最终结果为本次工作树 |
...
| 问题上下文 | [MagiesTerminal #2121](https://github.com/Zhangwei930/MgTerminal/issues/2121) |
...
- [README 与启动边界](../README.md#what-is-moshmagies)
...
| 项目 | 官方行为 | 当前 MoshMagies | 分类 | 判断与影响 |
|---|---|---|---|---|
| SSH 启动 | 论文第 2 页和 `mosh.pl`：SSH 启动非特权 `mosh-server`，服务端分配 UDP 端口和随机 128 位密钥，输出 `MOSH CONNECT <port> <key>` 后 SSH 退出 | MoshMagies 只接受 `host + port + MOSH_KEY`，不启动 SSH；README 明确把 SSH bootstrap 交给 MagiesTerminal/调用方 | **有意简化** | 这是模块边界，不是 UDP 协议缺失。MoshMagies 单独无法修复“没有收到 MOSH CONNECT” |
| 握手行解析 | 官方启动器只接受端口和 22 字符标准 Base64 密钥，并从 SSH 输出中提取远端地址 | MoshMagies 不解析该行；只在本地进程启动后读取环境变量 | **有意简化** | #2121 中 `no MOSH CONNECT` 发生在 MoshMagies 启动前；此时服务端看不到 UDP 包是预期结果，不能据此归因到 OCB/SSP |
...
| 地址与 locale 交接 | `mosh.pl` 固定 SSH 实际连接地址，避免 DNS 二次解析选到另一入口；同时处理 IPv4/IPv6、locale、远端命令、颜色数和端口范围 | MagiesTerminal 现在从 `SSH_CONNECTION` 取出实际服务器地址，同时保留原主机名作退路；MoshMagies 会尝试全部解析结果并在首个合法状态后锁定有效路径 | **明确兼容（功能子集）** | 多地址、双栈、SSH 别名和公网映射不再只依赖 DNS 的第一个结果；颜色数和高级启动参数仍由 MagiesTerminal 当前界面范围决定 |
...
| 项目 | 官方/RFC 行为 | 当前 MoshMagies | 分类 | 判断与影响 |
|---|---|---|---|---|
...
| 项目 | 官方/论文行为 | 当前 MoshMagies | 分类 | 判断与影响 |
|---|---|---|---|---|
...
| 宽字符下的隐藏内容 | 官方 framebuffer 允许在宽字符续格保存不可见内容，但 Display 增量画面不一定发送该内容；服务端和 xterm.js 对部分字符宽度也不同 | 当前按 MagiesTerminal 实际 xterm.js 宽度重建已收到的画面；不猜测官方未发送的隐藏内容 | **上游继承限制** | 连续画面与一次性最终画面不能作为宽字符场景的唯一判据；改用实际产品链路后，2,000 组、共约 36 万次操作与官方画面全部一致 |
...
- 官方 1.4.0 终端状态机在 macOS 和 Ubuntu 各生成 5,000 组确定性画面变化，重建后的 MagiesTerminal 终端逐格一致；另对 Ubuntu emoji 宽度差异做两组 xterm.js 定向验证，宽字符后的清除和后续字符位置均正确。
- 新增连续画面/一次性最终画面对照：Ubuntu 官方状态机下，20,000 组不含宽字符的随机画面为 0 差异。最终又把 MoshMagies 的逐次重绘与官方逐次重绘分别送入 MagiesTerminal 实际 xterm.js，2,000 组、每组 180 次操作为 0 个用户可见差异；三个可由客户端修正的右边界问题均已留下最小回归。
- MagiesTerminal 完整自动测试以受控并发运行：5,161 项通过、0 项失败、3 项按平台跳过；本轮 SSH 启动、地址交接和会话切换的 56 项定向测试全部通过。
...
### 4. 本地 MoshMagies 到公网 Ubuntu 官方服务端

最终拓扑与真实产品一致：本地 macOS 运行 release 版 MoshMagies，Ubuntu 机器只运行系统官方 `mosh-server 1.4.0`。
...
| IPv6、实际 MTU 1280 | 隔离链路两端分别运行 MoshMagies 和官方服务端，连接、中文/emoji、退格和退出均正确 |
...
- SSH 语言环境：MagiesTerminal 不再用 `LC_ALL` 强行启动远端服务，而是按官方包装器的固定顺序把 `LANG`、`LANGUAGE` 和各项 `LC_*` 作为候选交给 `mosh-server`；Ubuntu 24.04 上已证明缺失候选语言时仍可保留远端可用的 `C.UTF-8`，避免握手阶段无故失败。

## 与 MagiesTerminal #2121 的对应关系

| #2121 现象 | 本审计能确认的边界 |
|---|---|
| `no MOSH CONNECT` / 服务端看不到 UDP | 前者发生在 SSH 启动阶段，现已恢复官方 `-n -tt` 和完整候选语言语义，并覆盖分段、无换行和退出收尾；后者还可能来自 SSH 与 UDP 对同一多地址域名选择了不同入口。MagiesTerminal 现会交接 SSH 实际地址并保留原主机名退路，MoshMagies 会并行尝试候选并锁定首个合法路径。Ubuntu 连续 50 次启动、缺失语言环境和真实错误首选地址测试均通过；仍需在报告问题的 Windows 机器上做最终确认 |
| 偶发连接后又断 | 已修复建立会话后 60 秒退出、无效心跳和临时网络错误直接退出；公网 65 秒完全断网和旧路径失效都能恢复。SSH 启动阶段的偶发失败属于上层握手桥，应继续由 MagiesTerminal 单独覆盖 |
...
MoshMagies 的核心方向可以保留，不需要大范围重写。对照官方实现后发现的核心差异已经修复，并在“本地客户端连接公网 Ubuntu 官方服务端”的真实拓扑下通过了高延迟、丢包、乱序、重复包、长断网和换端口测试。
