...
在公网官方服务端上，往返约 800 ms、双向各 10% 丢包、乱序和 1% 重复包同时存在时，带空格输入、退格纠错、中文/emoji、长行、窗口缩放和清屏后重画全部通过。另用逐字节记录程序核对了完整输入序列，发送与接收完全一致。追加的 180×70 大画面测试完整重建了至少 40 行、每行 140 字节的确定性低压缩率内容，并从加密数据报中确认服务端确实发出同一指令的多个分片；12 秒双向静默黑洞后，同一会话也能恢复并继续执行命令。
...
- `cargo test --all-targets`：264 项通过、0 项失败；四个需要真实服务端的 live test 默认 ignored，并已用本机到该 Ubuntu 主机的连接单独执行通过；
...
| 双向静默丢包 12 秒后恢复 | 客户端不中止；恢复后同一会话继续执行命令并完成关闭确认 |
