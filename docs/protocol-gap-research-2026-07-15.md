# MoshMagies 协议差异补充研究（2026-07-15）
...
| SSH 与 UDP 可能选中同一域名的不同地址 | 已修复；MagiesTerminal 交接 SSH 实际地址并保留原主机名，MoshMagies 尝试全部候选，认证后锁定有效路径 |
...
- MoshMagies：`f2bb6ad7070d4f81ffd5c3592c98c94c75a39950`；这是补充研究开始时的固定对照点；
...
- Windows：根据 MagiesTerminal 的实际发布方式验证 WER/minidump 策略，不应照搬 Unix API 后就宣称跨平台完成。
